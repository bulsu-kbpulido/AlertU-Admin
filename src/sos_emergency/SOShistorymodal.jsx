...
/**
 * SOS Emergency Past History Viewer Modal
 * OpenLayers + OpenStreetMap (OSM) with live fallback resolution for Phone, Email & Emergency Contacts.
 */
export default function SOShistorymodal({
  sosId,
  targetRoom,
  citizenName,
  submitterName,
  ...
  const extractCoordinates = useCallback((loc) => {
    ...
    let timeString = 'Recorded Ping';
    const rawTime = loc.updatedAt || loc.triggeredAt || gis.updatedAt;
    if (rawTime) {
      if (typeof rawTime.toDate === 'function') {
        timeString = rawTime.toDate().toLocaleString();
      } else if (rawTime.seconds) {
        timeString = new Date(rawTime.seconds * 1000).toLocaleString();
      } else {
        timeString = new Date(rawTime).toLocaleString();
      }
    }
    ...
    return {
      lat: Number(lat),
      lng: Number(lng),
      address: loc.address || loc.formattedAddress || gis.address || 'Coordinates Recorded',
  ...
  const createMarkerStyle = useCallback(() => {
    const svgPin = `
      <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="#EA580C" stroke="#FFFFFF" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
  ...
  // 🎯 TARGETED FIRESTORE doc() LISTENER ON sos_alerts HISTORY
  useEffect(() => {
    ...
    const unsubscribeDoc = onSnapshot(
      ...
      (error) => {
        console.error(`❌ Error reading snapshot for sos_alerts/${activeSosId}:`, error);
        setIsDataLoaded(true);
      }
    );
      ...
      >
        <motion.div
          ...
          className={`relative bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-slate-200 ${
            isMinimized 
              ? 'w-[360px] border-2 border-orange-300 shadow-xl' 
        ...
        >
          ...
          {isMinimized ? (
            ...
            <div
              ...
              className="px-3 py-2 bg-orange-50 hover:bg-orange-100/80 border-b border-orange-200 flex items-center justify-between cursor-grab active:cursor-grabbing transition-colors"
            >
            ...
            <header className="px-5 py-3.5 border-b border-slate-200 bg-orange-50/60 flex items-center justify-between gap-3 shrink-0">
              <div className="flex items-center gap-3 truncate">
                <div className="p-2 rounded-xl bg-orange-100 border border-orange-200 text-orange-600 shrink-0">
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <button
                  ...
                  title={!isDataLoaded ? "Loading history record..." : "Close History"}
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
            ...
            <div className={`relative flex-1 bg-slate-200 flex items-center justify-center overflow-hidden ${
              ...
              {!isDataLoaded && (
                <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-white/90 backdrop-blur-sm p-6 text-center text-slate-600">
                  ...
                  <p className="text-sm font-bold text-slate-800">Loading History Location...</p>
                  <p className="text-xs text-slate-500 mt-1">Fetching saved GPS location snapshot.</p>
                </div>
              )}
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      <p className="text-xs font-bold text-slate-800 line-clamp-2">
                        {currentLocation.address || 'Address snapshot recorded'}
                      </p>
            ...
            {!isMinimized && (
              <div className="w-full lg:w-80 bg-white border-t lg:border-t-0 lg:border-l border-slate-200 p-5 flex flex-col justify-between shrink-0 space-y-4">
                ...
                <div className="pt-3 border-t border-slate-100 flex items-center justify-center gap-1.5 text-[11px] text-slate-500">
                  ...
                  <span>Timestamp: {currentLocation?.updatedAt || 'N/A'}</span>
                </div>
              </div>
            )}
          </div>
