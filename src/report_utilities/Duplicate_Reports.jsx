...
export default function Duplicate_Reports() {
  ...
  const handleRestore = async () => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        alert(result.message || "Failed to restore duplicate report.");
      }
    } catch (err) {
      console.error("Restore error:", err);
      alert("Failed to restore duplicate report. Please try again.");
    } finally {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/duplicate-reports/${selectedReportId}`, {
        method: 'DELETE',
        headers: headers
      });
      const result = await response.json();
  ...
  const handleConfirmBatchDelete = async () => {
    ...
    try {
      ...
      const deletePromises = idsArray.map((id) =>
        fetch(`${API_BASE_URL}/duplicate-reports/${id}`, { 
          method: 'DELETE',
          headers: headers
        })
      );
        ...
        <table className="w-full text-left border-collapse text-sm">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50/50 dark:border-slate-800 dark:bg-slate-800/50 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">
              ...
              <th className="px-6 py-4">Flagged Location</th>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span className="inline-block rounded-md bg-amber-50 dark:bg-amber-950/40 px-2.5 py-1 text-xs font-bold tracking-wide text-amber-800 dark:text-amber-300 border border-amber-200 dark:border-amber-900/50">
                    ...
                    <td className="px-6 py-4">
                      <div className="font-medium text-slate-900 dark:text-slate-100 capitalize">
                        {report.reportTitle || report.incidentType || report.hazard || 'Duplicate Incident'}
                      </div>
                    </td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
            ...
            >
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
                <div className="flex items-center gap-2">
                  <span className="rounded-md bg-amber-100 px-2.5 py-1 text-xs font-bold text-amber-800 dark:bg-amber-950/60 dark:text-amber-300">
                  ...
                  <h3 className="font-bold text-slate-900 dark:text-white capitalize">
                    {viewingReport.reportTitle || viewingReport.incidentType || viewingReport.hazard || 'Duplicate Incident'}
                  </h3>
                </div>
                <button
              ...
              <div className="mt-4">
                <ViewDuplicate_Reports report={viewingReport} onClose={() => setViewingReport(null)} />
              </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2 text-rose-600 dark:text-rose-400">
              <Trash2 className="h-5 w-5" /> Permanently Delete Duplicate Record?
            </AlertDialogTitle>
            <AlertDialogDescription>
              CRITICAL: This action <strong>cannot be undone</strong>. The duplicate incident record will be permanently purged from database archives.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={isSubmitting}>Cancel</AlertDialogCancel>
